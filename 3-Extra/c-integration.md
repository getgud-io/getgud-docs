...
To create a Custom Event Action, use the `SendCustomEventAction` function.
...
int SendCustomEventAction(struct BaseActionData baseData,
    ...
    int payloadSize);
```
* `baseData` - See BaseActionData. Set `playerGuid` to `NULL` with `playerGuidSize` 0 for a match-level event that belongs to no player, it is sent as `PvE`.
...
* `payload` - the event data, ideally a JSON object, though any string works. It must not be empty. `payloadSize` is used as the length, so the payload may contain any bytes. The SDK base64 encodes it before it enters the action stream, and Getgud decodes it back. Max 8192 chars after encoding, about 6 KB of raw data.
